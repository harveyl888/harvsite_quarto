...
The output is not we expect.  The points are in the right place but the order in which the lines are drawn between them does not follow the order of the x-axis.  This is a known issue with plotly, due to the fact that the x-values must be ordered for the lines to be drawn correctly.  A simple `arrange(ADY)` step will take care of this and a slighly edited prompt leads to a more satisfactory output.
...
Repsonse: *The issue is that error_y arrays don't get correctly split when color creates separate traces — each trace gets the full unsplit error array. The workaround is to build traces manually per group*
